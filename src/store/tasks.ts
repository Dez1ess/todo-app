...
export const addTaskAsync = createAsyncThunk(
  ...
  async (payload: AddTaskPayload) => {
    const response = await axios.post("http://localhost:3000/user/tasks", {
      id: payload.id, 
      title: payload.title,
...
export const removeTaskAsync = createAsyncThunk(
  ...
  async (payload: RemoveTaskPayload) => {
    await axios.delete(`http://localhost:3000/user/tasks/${payload.userId}/${payload.id}`);
    return payload.id;
  }
);
...
export const completeTaskAsync = createAsyncThunk(
  ...
  async (payload: CompleteTaskPayload) => {
    await axios.put(`http://localhost:3000/user/tasks/${payload.userId}/${payload.id}`);
    return payload.id;
  }
);
